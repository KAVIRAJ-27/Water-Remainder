...
import { validateDrinkAmount, mlToDisplayValue } from '../../utils/unitUtils';
...
interface DefaultAmountModalProps {
  ...
  unit: UnitPreference;
  onClose: () => void;
  onSave: (newAmountMl: number) => void;
}

const PRESET_AMOUNTS_ML = [150, 200, 250, 300, 500];

export const DefaultAmountModal: React.FC<DefaultAmountModalProps> = ({
  visible,
  currentAmountMl,
  unit,
  onClose,
  ...
  const [inputVal, setInputVal] = useState(() => mlToDisplayValue(currentAmountMl, unit).toString());
  const [errorMessage, setErrorMessage] = useState<string | null>(null);

  if (visible !== prevVisible) {
    setPrevVisible(visible);
    if (visible) {
      setInputVal(mlToDisplayValue(currentAmountMl, unit).toString());
  ...
  const handleSave = () => {
    const res = validateDrinkAmount(inputVal, unit);
    if (!res.isValid) {
      setErrorMessage(res.errorMessage || 'Invalid drink amount.');
      return;
    }
    onSave(res.amountMl);
    onClose();
              ...
              >
                <View style={styles.modalHeader}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={[styles.modalSubtitle, { color: colors.textSecondary }]}>
                      Standard volume for quick drinking
                    </Text>
                  </View>
                </View>
                ...
                <View style={[styles.inputContainer, { borderColor: errorMessage ? colors.danger : colors.border, backgroundColor: colors.surfaceElevated }]}>
                  <TextInput
                    ...
                    placeholder={unit === 'L' ? '0.25' : '250'}
                    placeholderTextColor={colors.textMuted}
                    keyboardType="decimal-pad"
                    selectTextOnFocus
                    accessibilityLabel="Default drink amount input"
                  />
                ...
                <View style={styles.modalButtons}>
                  ...
                  <TouchableOpacity
                    ...
                    activeOpacity={0.8}
                  >
                    <Text style={styles.btnSaveText}>Save Amount</Text>
                  </TouchableOpacity>
                </View>
              </View>
            </TouchableWithoutFeedback>
